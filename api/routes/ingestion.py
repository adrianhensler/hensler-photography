...
async def ingest_image(file: UploadFile = File(...), user_id: int = Form(...)):
    ...
    if len(contents) > max_size:
        ...
        error = file_too_large_error(
            file_size=len(contents), max_size=max_size, filename=file.filename, context=context
        )
        return JSONResponse(status_code=error.http_status, content=error.to_dict())
    ...
    try:
        ...
        variants, variant_error = generate_variants(
            str(file_path), original_filename, context={**context, "user_id": user_id}
        )
...
async def list_images(
    ...
    async with get_db_connection() as db:
        ...
        if search:
            count_query += (
                " AND (title LIKE ? OR caption LIKE ? OR description LIKE ? OR tags LIKE ?)"
            )
...
async def update_image_metadata(image_id: int, metadata: ImageMetadataUpdate):
    ...
    async with get_db_connection() as db:
        ...
        query = (
            f"UPDATE images SET {', '.join(updates)}, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
        )
        ...
        async with get_db_connection() as db:
            ...
            if not row:
                error = not_found_error(
                    resource_type="image", resource_id=image_id, context=context
                )
                return JSONResponse(status_code=error.http_status, content=error.to_dict())
...
async def regenerate_ai_metadata(
    ...
    try:
        ...
        logger.error(
            f"Failed to regenerate AI metadata: {e}", extra={"context": context}, exc_info=True
        )
        error = database_error(
